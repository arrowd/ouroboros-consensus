...
closeForkerEnv :: MonadSTM m => ForkerEnv m l blk -> m ()
closeForkerEnv ForkerEnv { foeWasCommitted, foeTracer, foeBackingStoreValueHandle } = do
  readTVarIO foeWasCommitted >>= traceWith foeTracer . bool ForkerCloseUncommitted ForkerCloseCommitted
...
implForkerCommit env = do
  guardUncommittedSTM env $ \dblog -> do
