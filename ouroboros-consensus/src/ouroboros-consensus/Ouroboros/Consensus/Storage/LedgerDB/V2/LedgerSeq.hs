...
deriving instance NoThunks (l EmptyMK) => NoThunks (StateRef m l)
...
deriving newtype instance NoThunks (l EmptyMK) => NoThunks (LedgerSeq m l)
...
applyThenPush :: (Monad m, ApplyBlock l blk)
              => LedgerDbCfg l
              -> blk
              ->    LedgerSeq m l
              -> m (LedgerSeq m l)
...
applyBlock :: forall m l blk. (ApplyBlock l blk, Monad m)
           => LedgerCfg l
           -> blk
           -> LedgerSeq m l
           -> m (StateRef m l)
applyBlock cfg b db = do
