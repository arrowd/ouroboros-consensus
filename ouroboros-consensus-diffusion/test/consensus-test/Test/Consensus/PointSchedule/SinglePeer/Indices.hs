...
headerPointSchedule g msgDelayInterval xs =
  ...
  where
    ...
    genHPBranchSchedule (tLast, trunkNextHp) ((Nothing, firstTip:_), mtMax) = do
      (p, tsTrunk) <- generatePerTipPointTimes mtMax (tLast, trunkNextHp) firstTip
      pure (p, HeaderPointSchedule tsTrunk [])
    ...
    generatePerTipPointTimes mtMax (tLast0, nextHp0) (tTip, tp) = go (max tLast0 tTip) nextHp0 []
      where
